  ...
  return (
    <div dir={dir} className="min-h-screen bg-[#FAFBFC]" style={{ fontFamily:"'Tajawal','Segoe UI',sans-serif" }}>
      ...
      <nav className={`glass-nav sticky top-0 z-50 transition-all duration-500 ${scrolled?'shadow-2xl':''}`}>
        ...
        {mob&&<div className="lg:hidden pb-4 animate-slideDown"><div className="bg-white/5 rounded-xl p-2">{nav.map((n,i)=><Link key={i} to={n.href} onClick={()=>setMob(false)} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${n.k==="n_lib"?"text-white bg-white/10":"text-white/80 hover:bg-white/5"}`}><span>{t(n.k)}</span></Link>)}</div></div>}
        </div>
      </nav>
