...
export default function ManafaaHajjPage(){
  ...
  return(<div dir={dir} className="min-h-screen bg-[#FAFBFC]" style={{fontFamily:"'Tajawal','Segoe UI',sans-serif"}}>
    ...
    <nav className={`gn sticky top-0 z-50 transition-all duration-500 ${scrolled?'shadow-2xl':''}`}><div className="max-w-7xl mx-auto px-4 sm:px-6"><div className="flex items-center justify-between h-16 sm:h-20">
      ...
      <div className="hidden lg:flex items-center gap-1">{nav.map((n,i)=><a key={i} href="#" className={`ni text-sm px-3 py-2 rounded-lg ${n.k==="n_hajj"?"text-white bg-white/10":"text-white/70 hover:text-white hover:bg-white/5"}`}>{t(n.k)}</Link>)}</div>
    ...
    {mob&&<div className="lg:hidden pb-4" style={{animation:'sd .3s ease-out'}}><div className="bg-white/5 rounded-xl p-2">{nav.map((n,i)=><a key={i} href="#" onClick={()=>setMob(false)} className={`flex items-center gap-3 px-4 py-3 rounded-lg ${n.k==="n_hajj"?"text-white bg-white/10":"text-white/80 hover:bg-white/5"}`}>{t(n.k)}</Link>)}</div></div>}
    </div></nav>
