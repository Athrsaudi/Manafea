...
export default function ManafaaHajjPage(){
  ...
  return(<div dir={dir} className="min-h-screen bg-[#FAFBFC]" style={{fontFamily:"'Tajawal','Segoe UI',sans-serif"}}>
    ...
    <nav className={`gn sticky top-0 z-50 transition-all duration-500 ${scrolled?'shadow-2xl':''}`}><div className="max-w-7xl mx-auto px-4 sm:px-6"><div className="flex items-center justify-between h-16 sm:h-20">
      <div className="flex items-center gap-3"><div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center" style={{background:'#C8A951',color:'#0F2530'}}><span className="text-lg sm:text-xl font-bold quran-font">م</span></div><div className="hidden sm:block"><h1 className="text-white font-bold text-base">{t("site_name")}</h1><p className="text-xs" style={{color:'#C8A951'}}>{t("site_desc")}</p></div></div>
      <div className="hidden lg:flex items-center gap-1">{nav.map((n,i)=><Link key={i} to={n.href} className={`ni text-sm px-3 py-2 rounded-lg ${n.k==="n_hajj"?"text-white bg-white/10":"text-white/70 hover:text-white hover:bg-white/5"}`}>{t(n.k)}</Link>)}</div>
    ...
    {mob&&<div className="lg:hidden pb-4" style={{animation:'sd .3s ease-out'}}><div className="bg-white/5 rounded-xl p-2">{nav.map((n,i)=><Link key={i} to={n.href} onClick={()=>setMob(false)} className={`flex items-center gap-3 px-4 py-3 rounded-lg ${n.k==="n_hajj"?"text-white bg-white/10":"text-white/80 hover:bg-white/5"}`}>{t(n.k)}</Link>)}</div></div>}
    </div></nav>
