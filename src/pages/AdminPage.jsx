        ...
        <div className="modal-overlay" onClick={()=>setModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {/* غلاف الكتاب */}
              <div className="form-group">
                <label className="form-label">غلاف الكتاب — ارفع من جهازك</label>
                <input type="file" accept="image/*" className="form-input" style={{padding:"8px"}}
                  onChange={e=>setForm({...form,coverFile:e.target.files[0]})} />
                {form.coverFile && <p style={{fontSize:".75rem",color:"var(--g)",marginTop:4}}>🖼️ {form.coverFile.name}</p>}
              </div>
              <div className="form-group">
                <label className="form-label">أو رابط الغلاف مباشر</label>
                <input className="form-input" placeholder="https://..." value={form.cover_url||""} onChange={e=>setForm({...form,cover_url:e.target.value})} />
              </div>
              {(form.coverFile || form.cover_url) && (
                <div style={{marginBottom:14}}>
                  <img src={form.coverFile ? URL.createObjectURL(form.coverFile) : form.cover_url}
